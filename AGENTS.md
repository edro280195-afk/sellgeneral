...
> **Repo:** este es `sellgeneral` (frontend) — la copia de trabajo de Regi Bazar que se está convirtiendo en plataforma **multi-tenant**. El backend vive en `sellgeneral-api`. Los repos ORIGINALES (`regibazar-web` y `api/EntregasApi`) están **congelados y no se tocan**.

## Qué es
Sistema de gestión de entregas para un negocio de ventas en vivo por Facebook. Los pedidos se capturan durante transmisiones en vivo ("Live Mode"), se gestionan en un tablero Kanban, y se entregan por un solo conductor con seguimiento en tiempo real. **Hoy es single-tenant** (un solo negocio, un solo conductor) — ver la sección "Single-tenant hoy".
